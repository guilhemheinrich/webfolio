...
--#region update short_description
CREATE
OR REPLACE FUNCTION upsert_short_description (
    query_experience_slug text,
    language_tag VARCHAR(2),
    new_content TEXT
) RETURNS VOID AS $$
...
BEGIN
    -- Vérifie s'il existe une traduction pour cette expérience et cette langue
    SELECT id INTO translation_id
    FROM webfolio_experience_short_description
    WHERE webfolio_experience_short_description.experience_slug = query_experience_slug AND webfolio_experience_short_description.language_code = language_tag;

    -- Si la traduction existe, on la met à jour
    IF FOUND THEN
        UPDATE webfolio_experience_short_description
        SET content = new_content
        WHERE id = translation_id;
    ELSE
        -- Sinon, on crée une nouvelle traduction
        INSERT INTO webfolio_experience_short_description (language_code, content, experience_slug)
        VALUES (language_tag, new_content, query_experience_slug)
        RETURNING id INTO translation_id;
    END IF;
END;
